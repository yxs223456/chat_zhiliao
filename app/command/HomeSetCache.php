 ...
 * 用户需同时满足在线、有相册
 */
class HomeSetCache extends Command
{
    use CommandTrait;
    ...
    {
        // setName 设置命令行名称
        $this->setName('chat_zhiliao:HomeSetCache');
    }
...
//            loginAndLogoutCallbackConsumer([$this, 'receive']);
            ...
            while(time() - $this->beginTime <= $this->maxAllowTime) {
                $data = homeSetCacheCallbackConsumer($redis);
    ...
    private function addCity($userId)
    {
        $redis = Redis::factory();

        // 如果用户设置了相册，把用户放入首页推荐列表
    ...
    private function popCity($userId, $city)
    {
        $redis = Redis::factory();
        $userInfo = UserInfoService::getUserInfoById($userId, $redis);
        ...
        foreach ($allConditions as $condition) {
            ...
            deleteUserFromHomeSiteList($userId, $city, $condition, $redis);
        }

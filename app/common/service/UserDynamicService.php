...
class UserDynamicService extends Base
{
    ...
    public function post($content, $source, $user)
    {
        ...
        try {
            ...
            $id = Db::name("user_dynamic")->insertGetId($dynamicData);
            $dynamicCountData = [
                'u_id' => $user["id"],
                'user_dynamic_id' => $id,
                'create_time' => date("Y-m-d H:i:s")
            ];
            Db::name("user_dynamic_count")->insert($dynamicCountData);
            Db::commit();
        } catch (\Throwable $e) {
            Db::rollback();
            throw $e;
    ...
    {
        $dynamic = Db::name("user_dynamic")->where("id", $id)->find();
        if (empty($dynamic)) {
        ...
        return Db::name("user_dynamic")->where("id", $id)->update(["is_delete" => DbDataIsDeleteEnum::YES]);
    ...
    public function personal($startId, $pageSize, $userId)
    {
        ...
        // 获取动态数据
        $dynamicQuery = Db::name("user_dynamic")
            ->where("u_id", $userId)
        ...
        $dynamicCount = Db::name("user_dynamic_count")

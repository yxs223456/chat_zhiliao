...
class VideoService extends Base
{
    ...
    {
        // 获取最新动态ID
        $newVideoId = Db::name("video")->alias("v")
     ...
     * @return array|mixed|null
     */
    public function recommendList($startId, $pageSize, $isFlush)
    {
        // 需要刷新更新缓存内容
        if ($isFlush) {
            return $this->recommendListUpdate($pageSize);
        }
        // 不需要刷新查询缓存数据
        return $this->recommendListPage($startId, $pageSize);
    }
     ...
     * @throws AppException
     */
    private function recommendListUpdate($pageSize)
    {
        // 获取最新动态ID
        $newVideoId = Db::name("video")
            ->where('is_delete', DbDataIsDeleteEnum::NO)
            ->order("id", "desc")
            ->value("id");
        // 为空，没有动态数据，返回缓存的第一页
        if (empty($newVideoId)) {
            return $this->recommendListPage(0, $pageSize);
        }

        // 不为空并且和缓存最新一条相同直接返回缓存数据
        $firstPage = $this->recommendListPage(0, $pageSize);
        $video = array_shift($firstPage);
        if (isset($video[0]["id"]) && $video[0]["id"] == $newVideoId) {
            return $this->recommendListPage(0, $pageSize);
        }
        ...
        // 不同删除城市小视频列表首页缓存
        deleteFirstRecommendVideoList($pageSize, Redis::factory());
        return $this->recommendListPage(0, $pageSize);
    }
     ...
     * @throws AppException
     */
    private function recommendListPage($startId, $pageSize, $retry = 0)
    {
        // 读缓存
        $redis = Redis::factory();
        if ($data = getRecommendVideoList($startId, $pageSize, $redis)) {
            return $data;
        }

        $lockKey = REDIS_KEY_PREFIX . "recommendVideoListLock:" . $startId . ":" . $pageSize;
        if ($redis->setnx($lockKey, 1)) {
            //设置锁过期时间防止失败后数据永修不更新
            $redis->expire($lockKey, Constant::CACHE_LOCK_SECONDS);
            $ret = [
            ...
            if (empty($videos)) {
                $redis->del($lockKey);
                cacheRecommendVideoList($startId, $pageSize, array_values($ret), $redis);
                return array_values($ret);
            }
            $ret["video"] = $videos;
        ...
        if ($retry < Constant::GET_CACHE_TIMES) {
            usleep(Constant::GET_CACHE_WAIT_TIME); // sleep 50 毫秒
            return $this->cityListPage($startId, $pageSize, ++$retry);
        }
        throw AppException::factory(AppException::TRY_AGAIN_LATER);
    }
}